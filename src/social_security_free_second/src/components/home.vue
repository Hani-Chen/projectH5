<template>
  ...
  <div class="page page-home">
    ...
    <!-- title -->
    <!-- Data: 2020-07-08; Edit: Hani; Describe: 增加普通领取状态; Start -->
    <div class="home-title" :class="{general:general}">
    <!-- Data: 2020-07-08; Edit: Hani; Describe: 增加普通领取状态; End -->
      <!-- title - 背景图 -->
      ...
      <div class="home-module title-content">
        ...
        <div class="ticket-wrap">
          ...
          <!-- Data: 2020-07-08; Edit: Hani; Describe: 金额添加小数点; Start -->
          <!-- 优惠券 - 金额 -->
          <div class="money">
            <div class="default" v-if="!isSwell">{{MoneyBefore}}</div>
            <div v-else>
              <div class="money-item-wrap money-item-wrap-0">
                <div class="money-item money-item-0">
                  <span>0</span>
                  <span>1</span>
                  <span>2</span>
                  <span>3</span>
              ...
              <span class="decimal" v-if="ticketMoney % 1 != 0">.</span>
              <div class="money-item-wrap money-item-wrap-1">
                <div class="money-item money-item-1">
                  <span>0</span>
                  <span>1</span>
                  <span>2</span>
                  <span>3</span>
          ...
          <!-- Data: 2020-07-08; Edit: Hani; Describe: 金额添加小数点; End -->
          <!-- 优惠券 - 文案 -->
          <div class="text-wrap">
            <!-- 标题 -->
        ...
        <div v-if="general">
          ...
          <button
            class="btn btn-waive"
            :class="[activeClass,{eventsnone:guide}]"
            @click="generalGet"
            @touchstart="touchstart('btn-waive-active')"
            @touchend="touchend()"
          >直接领取</button>
        </div>
        <div v-else>
          ...
          <button
            class="btn btn-swell"
            :class="[activeClass,{eventsnone:guide},{scale:swellAni}]"
            @click="swell"
            @touchstart="touchstart('btn-swell-active')"
            @touchend="touchend()"
          >
            {{swellText}}
    ...
    <div class="home-module home-lottery">
      ...
      <div v-if="lotteryResult == 1">
        ...
        <!-- 抽奖按钮 -->
        <button
          class="btn btn-lottery-same btn-lottery"
          :class="[activeClass,{eventsnone:guide}]"
          @touchstart="touchstart('btn-lottery-active')"
          @touchend="touchend()"
          @click="lottery"
        >
          立赢免单
      ...
      <div v-if="lotteryResult == 2">
        ...
        <!-- 分享按钮 -->
        <button
          class="btn btn-lottery-same btn-share"
          @click="lotteryShare"
          :class="activeClass"
          @touchstart="touchstart('btn-share-active')"
          @touchend="touchend()"
        >喊好友沾喜气</button>
      </div>
      ...
      <div v-if="lotteryResult == 3">
        ...
        <!-- 提醒我 -->
        <button
          class="btn btn-lottery-same btn-remind"
          :class="activeClass"
          @touchstart="touchstart('btn-remind-active')"
          @touchend="touchend()"
          @click="again"
        >激活医保电子凭证,再抽一次</button>
        <!-- 分享按钮 -->
        <button
          class="btn btn-lottery-same btn-share"
          :class="activeClass"
...
export default {
  data() {
    ...
    return {
      ...
      swellAni: false,
      isSwell: false,
      swellText: '激活医保电子凭证,金额立膨胀',
      // swellText1: '惊喜升级,点击金额立膨胀',
      // 优惠券标题
      ticketTitle: '药店返现券',
      ticketText:
  ...
  methods: {
    ...
    indexNumber() {
      ...
      setTimeout(function() {
        // // 防止有小数点 四舍五入
        // _this.ticketMoney = Math.round(_this.ticketMoney)

        if (_this.ticketMoney % 1 === 0) {
          // 是整数
          // 如果大于999 则 显示999
          individual = _this.ticketMoney % 10
          ten = parseInt(_this.ticketMoney / 10)
          ten >= 10 ? (ten = ten % 10) : ''
        } else {
          let number = _this.ticketMoney // number 是字符串 如果是数字需要先转成字符串 toString()
          individual = Number(String(number).split('.')[1])
          ten = Number(String(number).split('.')[0])
        }
        function numberMove(domeItem, time, value) {
          var demo = document.getElementsByClassName(domeItem)[0]
          setTimeout(function() {
            demo.style.transform = 'translate3d(0, -' + (10 + value) * 100 + '%, 0)'
          }, time)
        }
        numberMove('money-item-0', 30, ten)
        numberMove('money-item-1', 50, individual)
      }, 5)
    }
    /* Data: 2020-07-08; Edit: Hani; Describe: 金额添加小数点; End */
  },
