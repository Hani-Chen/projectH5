 ...
 * @param {string} tag
 */
function tarEvent(tag) {
  console.log(tag)
  tag && setTimeout(function(){
    M.tj.sendClick('nio20191211.'+tag)
  },0)
}
...
$('.btn_dynamic').on('click', function() {
  tarEvent('btn.dynamicposter')
  tarEvent('show.dynamiclist')
...
$('.make_wrap .btn_static').on('click', function() {
  tarEvent('btn.staticposter')
  tarEvent('show.staticbg')
  showSection($('.select_wrap'))
  // 判断是否已经生成swiper实例对象
  if (!mySwiperOwn) {
    mySwiperOwn = true
...
$('.select_base .btn_next').on('click', function() {
  tarEvent('staticposter.next')
  tarEvent('staticposter.staticlist')
  // 轮播禁止切换
...
$('.btn_create').on('click', function() {
  ...
  if (bgName == '' || bgText == '') {
    ...
    // 生成图片
    drawCanvas(bgImg, bgName, bgText, bgId)
  }
})
...
// canvas 生成落地页
// 文案
function drawCanvas(bgImg, bgName, bgText, bgId) {
  tarEvent('staticposter.create')
  ...
  ctx.fillStyle = '#fff'
  if (bgId == 3) {
    ctx.font = '35px BlueSkyNotoR'
  } else if (bgId == 4) {
    ctx.font = '35px BlueSkyNotoR'
  } else if (bgId == 5) {
    ctx.font = '38px BlueSkyNotoR'
  } else if (bgId == 6) {
    ctx.font = '45px BlueSkyNotoR'
  } else {
...
function initShare() {
  var share_data = {
    title: '与蔚来EC6一起仰望最美天空',
    wxtitle: '与蔚来EC6一起仰望最美天空',
    link: location.href,
    desc: '智能电动轿跑SUV蔚来EC6，现已开放预订',
...
musicReady(function() {
  M.client.invoke('ui', 'hideMiniPlayer')
})
