...
gulp.task('server', ['nodemon'], () => {
  ...
  let { port = 4000, protocol = 'http' } = argv
...
gulp.task('nodemon', callback => {
  ...
  $G.nodemon({
    ...
    env: { port, protocol, NODE_ENV: 'development', PROJ_NAME: project }
  }).on('start', () => {
    if (!called) {
      called = true
      callback()
...
gulp.task('staticCopy', ['clean'], () => {
  const project = proj_name
  const src_copy_path = `./src/${project}/{css,js,img,fonts,mp3,video,audio}/**/*`
