...
class FlowerFloat {
  ...
  // name() {
  //   var name = this.nameList[Math.round(Math.random() * (this.nameList.length - 1))]
  //   return name
  // }
  step() {
    /* 添加条件限制获取单个花瓣数据  */
    ...
    for (var i = 0; i < this.fallList.length; i++) {
      ...
      this.drawPetal(v.color, v.startX, v.startY, v.radius, v.startDeg, v.name)
   ...
   * @param {*} startDeg 花瓣角度
   */
  drawPetal(color, startSpeedX, startSpeedY, radius, startDeg, name) {
    this.cacheCtx.save()
    /* 通过清空子路径列表开始一个新路径的方法。 */
    ...
    if (this.form == 'fivePointed') {
      ...
      this.cacheCtx.font = "28px serif";
      this.cacheCtx.fillText(name, startSpeedX, startSpeedY)
    }
